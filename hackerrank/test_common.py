...
def test_true_for_all_should_be_true():
    ...
    truth_func: Callable[[int], bool] = lambda x: x % 2 == 0
    assert true_for_all(items, truth_func) is True


@pytest.mark.parametrize(
    "items, func, expected_func_result, expected",
    [
        ([2, 4, 6, 8], lambda x: x % 2 == 0, True, True),
        ([1, 2, 4, 6, 8], lambda x: x % 2 == 0, True, False),
    ],
)
def test_check_all(items: list[T], func: Callable[[T], bool], expected_func_result: bool, expected: bool):
...
@pytest.mark.parametrize(
    "numbers, start, length, expected",
    [([1, 2, 3], 0, 3, 6), ([1, 2, 3], 1, 2, 5), ([1, 2, 3, 4, 5], 1, 4, 14)],
)
def test_sum_desired_length(numbers: list[int], start: int, length: int, expected: int):
...
def test_numbers_to_counts(numbers: list[int], expected: dict[int, int]):
    assert numbers_to_counts(numbers) == expected


@pytest.mark.parametrize(
    "numbers, expected", [([0, 0, 3, 3], [0, 3]), ([0, 0, 3, 3, 7, 8, 8, 8], [0, 3, 7, 8])]
)
def test_dedupe_sequence(numbers: list[int], expected: list[int]):
    assert dedupe_sequence(numbers) == expected


@pytest.mark.parametrize("number, expected", [(0, True), (1, False), (2, True), (3, False), (4, True)])
